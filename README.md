# JanSeva Grievance Portal — AI Triage & Resolution System
...
| **Phase 3** | **Complaint Classification** | Dual TF-IDF + Soft-Voting Ensemble (Logistic Regression, Calibrated LinearSVC, SGDClassifier). |
| **Phase 4** | **Duplicate Detection** | Vector similarity using SentenceTransformer or hybrid word+character TF-IDF cosine fallback. |
| **Phase 5** | **Priority Prediction** | Urgency keyword rule safety-net + Calibrated LinearSVC ML predictor (**Critical, High, Medium, Low**). |
...
| **Phase 9** | **Evaluation & Auditing** | Master training & benchmark suite (`train_models.py` / `evaluate.py`) evaluating Accuracy, Macro-F1, and Language Bias. |

---

## Machine Learning & Model Handover Guide

For complete technical architecture, transformer fine-tuning steps, hyperparameter tuning, and collaborator handover instructions, see **[`ML_MODEL_GUIDE.md`](./ML_MODEL_GUIDE.md)**.
...
├── train_models.py               # Master CLI training & evaluation script
...
├── ML_MODEL_GUIDE.md             # Senior engineer ML status and training guide
├── data_store/
│   ├── grievances_db.json        # File-backed ticket database (with write-mutex guard)
│   ├── train.json                # Model training set (815 samples)
│   ├── val.json                  # Validation set (144 samples)
│   └── test.json                 # Uncontaminated test set (144 samples)
├── saved_models/
│   ├── baseline_classifier.pkl   # Classifier ensemble model artifact
│   └── priority_predictor.pkl    # Priority model artifact
